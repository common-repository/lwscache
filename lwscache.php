...
add_action('init', function () {
    ...
    load_plugin_textdomain('lwscache', false, dirname(plugin_basename(__FILE__)) . '/languages');
...
if ($type == "mutu") {
    if (isset($_SERVER['lwscache'])) {
        $cache_state = $_SERVER['lwscache'] == "On" ? "active" : "inactive";
    } else {
        $cache_state = "unavailable";
    }
} elseif ($type == "cpanel") {
    if (isset($_SERVER['HTTP_EDGE_CACHE_ENGINE']) && isset($_SERVER['HTTP_EDGE_CACHE_ENGINE_ENABLE'])) {
        if ($_SERVER['HTTP_EDGE_CACHE_ENGINE'] == "varnish") {
            $cache_state = $_SERVER['HTTP_EDGE_CACHE_ENGINE_ENABLE'] == "1" ? "active" : "inactive";
        } else {
            $cache_state = "unavailable";
        }
    } else {
...
$GLOBALS['lwscache_state'] = ['type' => $type, 'state' => $cache_state];
